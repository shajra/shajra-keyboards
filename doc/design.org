...
We get used to a key being in a poor place.  So, we must be diligent not to
confuse familiarity with ergonomic improvement.
...
Additionally, there are a few physical assumptions that factor into some
decisions:

- The original author has larger hands, which means some keys are assumed
  reachable, which may not be the case for everyone.
...
For reference, below are diagrams of the keymaps for the Model 01/100 and
Ergodox EZ.
...
With the Ergodox EZ, Moonlander, and Model 01/100, we get new keys that can get
much more utility from our thumbs/palms, which are sturdier than any of our
fingers.  Our thumbs/palms get home keys of their own (the Model 01/100 even has
physical homing bumps on its thumb keys). These keys are a central feature of
both keyboards.
...
still need to stretch for the key press.
...
Furthermore, we sometimes need two modifiers for chording and must use both
hands. If the two modifiers we needed were on the same split, we'd be out of
luck with only one thumb per hand.

Three-modifier chording does happen, but we only support such chords where one
of the three modifiers is a =Shift=. For this reason, we have =Shift= modifiers
on both thumb keys as well as their traditional outer keys. Thumbs of each hand
would press the other two modifiers, while one of the pinkies presses a =Shift=.
...
when pressing one of the yellow keys with our thumbs, all fingers can press a
key. If we use a pinky finger for a modifier, other keys normally depressed by
the pinky are less available for a one-handed chord. The same is true for any of
our fingers. When we use our thumbs for modifiers, more chords are ergonomically
available for one-handed chording in a home position.
